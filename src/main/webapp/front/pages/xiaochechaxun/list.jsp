                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="xiaochechaxunName" id="xiaochechaxunName" placeholder="校车编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../xiaochechaxun/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.xiaochechaxunPhoto?item.xiaochechaxunPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../xiaochechaxun/detail.jsp?id='+item.id)"><span>{{item.xiaochechaxunName}}</span></div>
                                <div class="ng-prslist-tit" v-if="item.xiaochechaxunNewMoney"><span style="color: red;">{{item.xiaochechaxunNewMoney}} RMB</span></div>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 xiaochechaxunContent 字段去除无用字符 如果字段长度大于60  就截取到60
            xiaochechaxunContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#xiaochechaxunName').val() != null && jquery('#xiaochechaxunName').val() != "" && jquery('#xiaochechaxunName').val() != "null" ) {
                param['xiaochechaxunName'] = jquery('#xiaochechaxunName').val();
            }
                                                    // 获取列表数据
            http.request('xiaochechaxun/list', 'get', param, function(res) {
