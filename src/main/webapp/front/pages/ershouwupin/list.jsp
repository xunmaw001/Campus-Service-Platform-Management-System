                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="ershouwupinName" id="ershouwupinName" placeholder="物品名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="ershouwupinTypes" id="ershouwupinTypes" lay-filter="ershouwupinTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in ershouwupinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../ershouwupin/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.ershouwupinPhoto?item.ershouwupinPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../ershouwupin/detail.jsp?id='+item.id)"><span>{{item.ershouwupinName}}</span></div>
                                <div class="ng-prslist-tit" v-if="item.ershouwupinNewMoney"><span style="color: red;">{{item.ershouwupinNewMoney}} RMB</span></div>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            ershouwupinTypesList : [],
            ershouwupinTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 ershouwupinContent 字段去除无用字符 如果字段长度大于60  就截取到60
            ershouwupinContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#ershouwupinName').val() != null && jquery('#ershouwupinName').val() != "" && jquery('#ershouwupinName').val() != "null" ) {
                param['ershouwupinName'] = jquery('#ershouwupinName').val();
            }
         
            var ershouwupinTypes = $("#ershouwupinTypes").find("option:selected").val();
            if (ershouwupinTypes != null && ershouwupinTypes != "" && ershouwupinTypes != "null") {
                param['ershouwupinTypes'] = ershouwupinTypes;
            }
                                                            // 获取列表数据
            http.request('ershouwupin/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function ershouwupinTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=ershouwupin_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.ershouwupinTypesList = res.data.list;
               }
           });
       }
